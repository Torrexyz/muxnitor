...
const MODALS_SRC_CACHE = { styles: {}, scripts: {} };
var REQUEST_MODAL_DELAY = false;

async function requestModal(url, config = {}) {
  if(REQUEST_MODAL_DELAY === false) {

    REQUEST_MODAL_DELAY = true;

    if(typeof event !== "undefined") {
      let targetNode = event.target;
      switch (targetNode.nodeName) {
        case 'path': targetNode = targetNode.parentNode.parentNode; break;
        case 'svg':  targetNode = targetNode.parentNode; break;
        default: targetNode = targetNode;
      }
    ...
    if(REQ.ok) {
      ...
      if(RES instanceof Element ? RES.nodeName === "MODAL" : false) {
        ...
        let loadScriptNodes = [];
        if(!MODALS_SRC_CACHE.scripts.hasOwnProperty(url)) {
          try {
            ...
            } else { MODALS_SRC_CACHE.scripts[url] = false; }
        ...
        if(!MODALS_SRC_CACHE.styles.hasOwnProperty(url)) {
          try {
            ...
            if(modalStyle.ok) {
              ...
              MODALS_SRC_CACHE.styles[url] = localStyleNode;
              loadStyleNode = localStyleNode;
            } else { MODALS_SRC_CACHE.styles[url] = false; }

          } catch {}
        } else if(MODALS_SRC_CACHE.styles[url] !== false) {
          loadStyleNode = MODALS_SRC_CACHE.styles[url];
          console.log(`(${url})\n  >> STYLE_LOAD_REMEMBERED`);
        }
        ...
        document.addEventListener("ModalDOMContentLoaded", function handler(evnt) {
          ...
          setTimeout(() => REQUEST_MODAL_DELAY = false, 500);
        });
