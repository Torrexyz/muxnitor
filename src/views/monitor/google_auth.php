  ...
  if(isset($google_token['access_token']) && $DOCUMENT_STATE_ERROR === false) {
    ...
    if($googleUserInfo->hd === 'utp.edu.co') {
      if(dbcursor("SELECT * FROM monitor WHERE `email` = '{$googleUserInfo->email}'")->rowCount() > 0) {
        ...
        if(dbcursor("INSERT INTO monitor(`folder`,`email`,`name`,`profile`) VALUES (?,?,?,?)", array_values([
          'folder'  => str_replace(['@utp.edu.co', '.'], ['', '_'], $googleUserInfo->email),
          ...
          @mkdir(__DATA_ROOT__.'/'.str_replace(['@utp.edu.co', '.'], ['', '_'], $googleUserInfo->email));
          session_start();
          $_SESSION['sessref'] = $googleUserInfo->email;
    ...
    <?php
      if($DOCUMENT_STATE_ERROR === false) {
        if($LOGIN_ACCOUNT_STATE === 1) {
          ...
          $domain_not_valid = $googleUserInfo->hd !== 'utp.edu.co' ? '<i style="display:block;font-size:12px">(Correo no académico)</i>' : null;
